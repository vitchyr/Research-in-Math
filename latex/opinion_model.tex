...
\begin{defn}
 Let $\overline{d}$ be the mean edge length (i.e. distance) when the graph has reached equilibrium.
\end{defn}
We posit that vertices $x$ such that $\min(x, 1 - x)$ is significantly greater than $\overline{d}$ have $\lambda \approx \overline{k}$; this is because almost none of the edges $\{x, u\}$ reach the boundary of the opinion space. On the other hand, vertices $x$ such that $\min(x, 1 - x)$ is relatively close to $\overline{d}$ have $\lambda < \overline{k}$ because the boundary of the opinion space prevents some edges $\{x, u\}$ that would otherwise be present. From Figure \ref{subfig:lambda-compare}, we see that the threshold $\min(x, 1 - x)$ at which $\lambda(x)$ begins to drop sharply decreases with $n$. This can be explained by considering the relationship between $\overline{d}$ and $n$. For finite $n$, $\overline{d} > 0$, since for $u$ and $v$ selected randomly from $\Omega$, $Pr(u = v) = 0$ and thus $d(u, v) > 0$. However, we will prove that, as $n$ tends to infinity with $\overline{k}$ held constant, $\overline{d}$ approaches 0. This explains why vertices very close to the boundary of the opinion space can have $\lambda \approx \overline{k}$ in large graphs.
\begin{prop}
For any dimension $D$,
 \begin{equation*}
  \lim\limits_{n\to\infty} \overline{d} = 0.
 \end{equation*}
\end{prop}
\begin{proof}
First we note that
\begin{equation}
\label{eqn:overline-d-sum}
 \overline{d} = \frac{1}{{n \choose 2}} \sum\limits_{e\in K_n}  P(e)\,d(e).
\end{equation}
From Theorem \ref{thm:perc}, we know that the probability $P(e)$ an edge $e$ is present at equilibrium is $\frac{c}{d(e)^2 + c}$. For large $n$, vertices are distributed evenly within the opinion space, so there are $n\, du$ vertices in the interval $du = du_1 \cdot du_2 \cdots du_D$ and $\frac{1}{2}n^2 \,du\, dv$ edges between vertices in $du$ and vertices in $dv$.  Thus for large $n$,
\begin{equation}
\label{eqn:overline-d-integral}
\overline{d} = c\int\limits_\Omega \int\limits_\Omega \frac{\,d(u, v)}{d(u, v)^2 + c}\,du\,dv
\end{equation}
since the $n \choose 2$ from Equation \ref{eqn:overline-d-sum} cancels with the $\frac{1}{2} n^2$ from $\frac{1}{2}n^2 \,du\, dv$. Compare this to Proposition \ref{prop:c}, which states that
\begin{equation*}
 \int\limits_\Omega \int\limits_\Omega \frac{du \, dv}{d(u, v)^2 + c} = \frac{2m}{c\,n^2}.
\end{equation*}
The limits of integration are the same, though the integrand of Equation \ref{eqn:overline-d-integral} has $d(u, v)$ in the numerator while the other does not. Because the maximum distance is $D$,
\begin{equation}
 \frac{\,d(u, v)}{d(u, v)^2 + c} \leq \frac{\,D}{d(u, v)^2 + c}
\end{equation}
for any $u, v$. Thus $\overline{d} \leq D\,\frac{2m}{n^2}$. Making the substitution $m = \frac{1}{2} \overline{k} \,n$ and taking the limit, we have $\lim_{n\to\infty} \overline{d} \leq D\,\overline{k}\,\lim_{n\to\infty}\frac{1}{n} = 0.$
\end{proof}
