...
We posit that vertices $x$ such that $\min(x, 1 - x)$ is greater than lengths of most edges have $\lambda \approx \overline{k}$; this is because almost none of the edges $\{x, u\}$ reach the boundary of the opinion space. On the other hand, vertices $x$ such that $\min(x, 1 - x)$ is not greater than the lengths of most edges have $\lambda < \overline{k}$ because the boundary of the opinion space prevents some edges $\{x, u\}$ that would otherwise be present. 

From Figure \ref{subfig:lambda-compare}, we see that the threshold $\min(x, 1 - x)$ at which $\lambda(x)$ begins to drop decreases with $n$. From Table \ref{tbl:dist-distro}, we see that --- as a general rule --- the mean edge distance decreases as $n$ increases; this is because, as vertices become more tightly-packed, shorter edges become possible. Thus for greater $n$, vertices can be closer to the boundary of the opinion space while still having $\lambda \approx \overline{k}.$
