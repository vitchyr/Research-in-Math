...
\begin{defn} 
For states $G$ and $H$ that meet the transition condition on $v_P$, define $C(G, v_P)$ to be the part of the transition probability $P(G, H)$ that is independent of $H$.
\end{defn}
...
\begin{prop}
 Let there be three states $G$, $H$, and $J$ such that each pair of states meets the transition condition on $v_P$. Then,
...
\begin{proof}
 For the break-function model, 

 For the rewire-function model, $f(G, J) = R(v_J \in G)$.
\end{proof}



Now we make one final definition:
...
\begin{prop}
 ...
 \begin{equation}
\label{eqn:pi-k1}
  \pi(G, v_P) = \frac{k_1}{C(G, v_P)} \prod \limits_{H \in \delta(G, v_P)} \frac{1}{f(G, H)}
 \end{equation}
...
\begin{equation}
 \frac{C(H, v_P)}{C(G, v_P)} \cdot \frac{f(H, J_0) \cdot f(H, J_1) \cdot f(H, J_2) \cdots f(H, G)}
{f(G, J_0) \cdot f(G, J_1) \cdot f(G, J_2) \cdots f(G, H)} = 
\frac{P(H, G)}{P(G, H)}
\end{equation}
